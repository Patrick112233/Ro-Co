...
case $ARCHITECTURE in
  x86_64)
    echo Testing cypress/included in amd64 using Chrome
      docker run --rm -v .:/app -w /app --entrypoint cypress cypress/included open -b chrome
    ;;
  aarch64)
    echo Testing cypress/included in arm64 using Firefox
      docker run --rm -v .:/app -w /app --entrypoint cypress cypress/included open -b firefox
    ;;
  *)
